...
public class DecreaseDefensesDebuff : MonoBehaviour, Effect
{
    ...
    void Start()
    {
        ...
        icon.target = this;
        icon.effect = "Decrease Defenses -" + (intensity * 100.0f).ToString() + "%";
        icon.sprite = "";
    ...
    public void effect()
    {
        target.defMultis.Add(intensity);
    }

    public void expire()
    {
        target.defMultis.Remove(intensity);
        icon.expire();
        Destroy(this);
    }
    public void onHit()
